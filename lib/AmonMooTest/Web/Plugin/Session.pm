package AmonMooTest::Web::Plugin::Session;
use strict;
use warnings;
use utf8;
...
my $cipher = Crypt::CBC->new({
    key => '92r1TOyYrpA1QiC9R4FIPb7azdy2OS8J',
    cipher => 'Rijndael',
});
sub _session {
    my $self = shift;
    ...
    if (!exists $self->{session}) {
        $self->{session} = HTTP::Session2::ClientStore2->new(
            env => $self->req->env,
            secret => '1zQup3Q9ub5-bbaz-56g6iIf0j5ns__0',
            cipher => $cipher,
        );
    }
    return $self->{session};
...
This module manages session for AmonMooTest.
